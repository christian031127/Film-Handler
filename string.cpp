...
String& String::operator+=(const String& rhs) {
    // Kiszámoljuk az új hosszt
    int newLength = strlen(pData) + strlen(rhs.pData);

    // Foglalunk memóriát az új stringnek, beleértve a lezáró nullát is
    char* temp = new char[newLength + 1];

    // Másoljuk az eredeti stringet az újba
    strcpy(temp, pData);

    // Fűzzük hozzá a másik stringet
    strcat(temp, rhs.pData);

    // Felszabadítjuk az eredeti string által foglalt memóriát
    delete[] pData;

    // A pData mutatót az új stringre állítjuk
    pData = temp;
...
String& String::operator+=(char rhs) {
    ...
    // Hozzáadjuk az új karaktert a végéhez
    temp[oldLength] = rhs;
    temp[newLength] = '\0';
...
bool operator==(const String& egyik, const String& masik) {
